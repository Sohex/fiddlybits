...
cross-encoder reranker, and report document recall, MRR and page hit at 10 before and after, plus latency per query.
    ...
    rerank_ab.py <embed-ab dir> <embedding model name> <reranker path> [reranker kind: qwen3|bge] [K]
"""
import json, pathlib, sys, time
import numpy as np, torch
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
import embed_ab as E
def load_reranker(path, kind):
    ...
    kw = dict(max_length=E.MAX_SEQ, device='cuda', model_kwargs={'torch_dtype': torch.float16})
...
def main():
    out, emb_name, rpath = pathlib.Path(sys.argv[1]), sys.argv[2], sys.argv[3]; kind = sys.argv[4] if len(sys.argv) > 4 else 'qwen3'; K = int(sys.argv[5]) if len(sys.argv) > 5 else 50
    ...
    score = load_reranker(rpath, kind); S = Q @ D.T
    def metrics(orders):
    ...
    res = {'embedding': emb_name, 'reranker': rpath, 'K': K, 'before': metrics(base), 'after': metrics(rer), 'rerank_seconds_per_query': lat}
    json.dump(res, open(out / f'rerank_{emb_name}_{pathlib.Path(rpath).name}.json', 'w'), indent=1)
    b, a = res['before'], res['after']
    print(f"| stage | doc recall@5 | doc recall@10 | MRR | page hit@10 |\n|---|---|---|---|---|\n| {emb_name} alone | {b['doc_recall@5']:.2f} | {b['doc_recall@10']:.2f} | {b['mrr']:.2f} | {b['page_hit@10']} |\n| + {pathlib.Path(rpath).name} (top {K}) | {a['doc_recall@5']:.2f} | {a['doc_recall@10']:.2f} | {a['mrr']:.2f} | {a['page_hit@10']} |\nrerank latency {lat:.1f} s/query")
