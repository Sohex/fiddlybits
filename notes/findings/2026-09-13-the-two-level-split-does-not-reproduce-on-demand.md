# area_fraction_above at level 7: exclusive access removes the split in the real bed; no neighbour tried reproduces the named levels on demand
...
## The real bed under exclusive access: one level, twenty for twenty
...
occupancy readings. This is the row's "split removed" branch, on the real bed rather
than a stand-in probe, from `main` merged in.
...
Requesting every share removes the ambiguity in the real bed: twenty for twenty, one
level, mean 85.475 us against `gpu-share`'s named 84 and 107. That result is adopted
as the operating rule regardless of mechanism (user direction 2026-09-13: any bench or
probe run whose timings are counted takes every share).

The mechanism the two historical campaigns' discrete, whole-run-persistent split
came from is not established by this investigation. Nine neighbour profiles,
spanning negligible to thirty-fold effects, real single-case probes and real full bed
copies alike, do not reproduce it as this case's own minimum-over-many-batches
statistic; nor does the tighter `vram` cap alone, without contention. What contention
plainly does do here (10x to 30x while an SM-saturating neighbour runs, recovering to
the floor the moment it and its lingering batch pass) is a different, larger and
non-discrete effect from the named levels, not a demonstration of them.
