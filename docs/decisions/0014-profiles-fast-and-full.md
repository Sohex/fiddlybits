...
- 2026-09-14: the profile carries `settle_interval`, the simulated seconds between the store's writer settling, a `Disposition{FT,TIME}` above zero with a disposition from `Systems.DECLARED`, required on `Profile`, `fast_profile` and `full_profile` with no default and no `Absent` (decision 0060, docs/plans/fiddlybits-52v.6-provenance.md, section "The writer", fiddlybits-52v.6.30, after fiddlybits-52v.6.25).
