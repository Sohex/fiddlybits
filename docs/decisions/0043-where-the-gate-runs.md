...
- 2026-09-14: the clean room and the gate split on a closed vocabulary of machine resources, not on a subset of suites. The vocabulary is `card`, `oracle_data`, `input_data` and `references`, each probed at the repository's main checkout; a fifth name is an amendment to this record. Every check that needs a resource calls one helper, `Resources.requiring` in `test/resources.jl`, and a suite, file or lint needing a resource throughout calls it once at its top. Where the resource is absent the helper records a named `NotEvaluated` result and prints it, never an error and never a silent skip; the result is its own name and not `Verdicts.NotEvaluable`, which is a loop verdict (decision 0009) and says nothing about a host. The one-process door `Pkg.test()` runs prints every check not evaluated by name and accepts one only for a resource named in `FIDDLYBITS_ABSENT_RESOURCES`, which `.github/workflows/clean-room.yml` sets to what the hosted runner lacks; an unset declaration excuses nothing, and a declared resource the probe finds present refuses the run as stale. The gate and the nightly declare nothing absent and refuse any check not evaluated, so a resource missing on this machine is a refusal and not a pass. The one-process door runs each suite in a module of its own, the isolation the gate has from processes (decision 0049). The row in What runs where for a hosted job reads: a clean checkout that resolves, loads, and passes every check except those the declaration excuses, which it names. The consequence that a check needing a dataset moves to the local gate is replaced: a check needing a dataset reads a small deterministic extract of exactly what it reads, committed under `oracles/extracts/` or `inputs/extracts/` with a recipe and hashes in the dataset's manifest where the dataset's redistribution licence permits, within the bounds `docs/oracles/extracts.toml` declares; it runs on the extract in both places, and the gate re-derives the extract from the payload and refuses a mismatch. Where the licence does not permit an extract, the check reads the payload through the helper and is named not evaluated in the clean room. The card has no substitute. The repository is public, so every manifest records its licence, where it was read and whether an extract is permitted. Alternatives weighed: a suite-level list of what the clean room skips, which lost because card arms share files and testsets with processor checks and the list cannot name the checks it drops; per-arm checks without a shared helper, the state this amendment replaces, which lost because nothing records, prints or judges what was not evaluated; a separate hosted job per resource, which lost because it still needs this selection and cannot separate interleaved arms; and a probe with no declaration, which lost because a resource the clean room should hold would go missing unseen. User decision of 2026-09-14, raised on `fiddlybits-unx`, from notes/findings/2026-09-14-the-clean-room-fails-on-the-card-and-on-two-causes-of-its-own.md. Planned in `docs/plans/fiddlybits-unx-machine-resources.md`; carried by `fiddlybits-qwi`, `fiddlybits-zyo`, `fiddlybits-d47`, `fiddlybits-8we`, `fiddlybits-6xt`, `fiddlybits-a0z` and `fiddlybits-0b5`, verified by `fiddlybits-hmt`.
