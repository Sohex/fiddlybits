...
- 2026-09-14: the references payload is recorded by a tracked hash manifest, `docs/references/payload.toml`, holding for every file under `references/pdf/` its path, bytes and sha256, and for every directory under `references/` an index row names a digest over its files' paths, bytes and sha256 with the pinned commit where it is a git checkout; it holds no content of any file, so the PDFs stay untracked on the size and licence grounds this record gives, and the repository being public changes nothing about that. `tools/references/payload_manifest.jl` writes it, and a fetched file's entry is committed with the index row that moves it to `held`. Two checks read it: the index lint of `fiddlybits-f1x` resolves every `held` or `read` row's file against the manifest and never against the disk, so it runs on a checkout with no payload; `build.references_payload_hashes` checks the manifest against the files on disk where the payload is, and is named not evaluated through the resource helper of decision 0043's amendment of the same date where it is not. The alternative weighed was the lint reading the disk alone, which lost because the clean room holds no payload, so the check could only fail there or be excused there, and a check excused in one place of two confirms nothing about the index on a public checkout. User decision of 2026-09-14, raised on `fiddlybits-unx`. Planned in `docs/plans/fiddlybits-unx-machine-resources.md`, section The references hash manifest; carried by `fiddlybits-0b5` and `fiddlybits-f1x`.
