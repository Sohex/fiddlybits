...
- 2026-09-14: rule 4, pattern and not global mean, is carried by the registry. Every tier-2 entry declares `form`, `pattern` when its statistic is itself spatial, seasonal or class structure or locates a feature of it, `scalar` when it is one number over the whole domain; a scalar entry names in `pattern_entry` a tier-2 entry of form `pattern`, its partner. Each entry keeps one statistic, one bar and one set of anchors, and the partner is judged and registered on its own. The loader refuses a tier-2 entry with no form or a form outside that closed set, a scalar entry with no `pattern_entry`, and a `pattern_entry` naming an entry that does not exist, is not tier 2 or is not of form `pattern`. The runner judges one statistic per entry against that entry's own bar, and the distance report shows a scalar beside its partner. A metric with no published pattern spread either gets a partner with its own sourced bar, or leaves tier 2. Three further rules, the user's answers of the same date: (1) a `fail_bar` scalar names a `fail_bar` partner, and a report scalar names a partner of either kind; (2) `form` and `pattern_entry` are fixed at registration, joining the fields clause (2) of the amendment of 2026-09-13 requires the `registered_at` commit to hold, and no merge on the mainline changes an entry's `form` or `pattern_entry` together with `src/` or its testset, the clause (4) rule for a threshold, because a partner re-pointed after its scalar's value is seen chooses the comparison with the value known; (3) a scalar and its partner share `holdout`, and the loader refuses a pair whose values differ, so no nightly shows a scalar with its pattern held out. The user also decided the scalars that had no known pattern source: `earth.ceres_toa_balance` moves to tier 1 as an energy-conservation criterion with its statistic unchanged; `earth.ecs_report` moves to tier 3 as a published spread with a protocol declared from its assessment; `earth.rgi_glacier_area` leaves tier 2 for tier 3 or removal as its sources decide; the others take partners on named sources. A tier move is a change to an existing entry, so it merges registry-only, apart from code. The alternatives weighed were one entry carrying both a scalar and a pattern statistic with a bar each, which lost because every entry would carry two bars while registration, anchors and verdicts are per bar (one `registered_at` threshold, one set of anchors and datasets to re-register on, one verdict semantics per entry by decision 0053); and a run-time check that a payload carries a pattern statistic, the first form of `fiddlybits-52v.8.3`, which lost because it enforced nothing at load and the pattern it demanded had no declared bar. The user chose this rule on 2026-09-14 (option B, raised on `fiddlybits-52v.8.3`). Planned by `fiddlybits-zar` in `docs/plans/fiddlybits-52v.8-oracles.md`, The form of a tier-2 entry, which classifies every tier-2 entry; carried by `fiddlybits-52v.8.13` to `fiddlybits-52v.8.23`.
- 2026-09-14: the basis of a bar. A bar is drawn from the published spread of comparable models' residuals against the observation where such a spread exists, and is never narrower than the observation's own uncertainty; where no published spread of model residuals exists, the observation's uncertainty alone is the basis. A pre-registration fixes a bar before the value it judges has been seen (the amendment of 2026-09-13) and is not a basis by itself: a pre-registered number states which of the two it rests on. This replaces, for the basis of a bar, the Tier 2 sentences that take a FAIL bar only from a published model's own residual and otherwise make the metric REPORT; a physical constraint stays a basis, and the tier-3 bar stays the inter-model spread of its protocol. User decision of 2026-09-14, raised by the source-fitness audit (notes/findings/2026-09-14-an-audit-of-source-fitness.md, where many registry bars rested on no stated basis, on an observational spread, or on a pre-registration alone). The registry cases are carried by `fiddlybits-iwq`, `fiddlybits-5r5`, `fiddlybits-n13`, `fiddlybits-te0`, `fiddlybits-pi4` and `fiddlybits-2tq`, and the tier-2 partner bars by `fiddlybits-52v.8.15`, `fiddlybits-52v.8.16`, `fiddlybits-52v.8.17` and `fiddlybits-52v.8.20`.
