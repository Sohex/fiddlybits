...
| `Sourced` whose identifier is not marked `read` in `docs/references/INDEX.md` | a fixture index row marked `held` |
...
The reference check reads `docs/references/INDEX.md` and takes the status column at
its word. That file is a live document and the check re-reads it, so a paper moving
from `held` to `read` arms every `Sourced` value that cites it without any value
being edited.
...
`Numerics` holds what is a fact about a run rather than about the system, and it is a
field of `System` rather than a separate argument so that the run identity of decision
0010 hashes one object.
...
a dry run on the smallest mesh. `declares(component)` returns what the component says
it reads. The test is `recorded` is a subset of `declared`, run over every component;
...
`affected(field)` answers "what re-runs when this changes" from the declared graph,
never from a list kept by hand (REQ-SYS-008). The declared sets are what artifact keys
...
| --- | --- | --- | --- |
| 52v.4.2 | sonnet | `src/Dispositions/`, `test/dispositions/` | `system.disposition_refusals` passes with every control firing, each named |
| 52v.4.3 | frontier | `src/Systems/` except `tracking.jl`, `test/system/` | every constructor refusal fires on its control, each named; `isbits(strip(system))`; `system.gravity_and_figure` passes |
